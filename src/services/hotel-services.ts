...
async function getHotels(userId: number) {
  ...
  if (!validUser) throw notFoundError();
  if (!validUser[0] || !validUser[1]) throw notFoundError();
  if (validUser[1].status !== 'PAID') throw paymentRequiredError();
  if (!validUser[1].TicketType.includesHotel) throw paymentRequiredError();
